...
CREATE TABLE IF NOT EXISTS `donate`.`report` (
  ...
  INDEX `fk_report_hospital1_idx` (`hospital_id` ASC),
  CONSTRAINT `fk_report_donor`
    FOREIGN KEY (`donor_id`)
    REFERENCES `donate`.`donor` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_report_hospital1`
    FOREIGN KEY (`hospital_id`)
    REFERENCES `donate`.`hospital` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- -----------------------------------------------------
-- Table `donate`.`tocken_details`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `donate`.`tocken_details` ;

CREATE TABLE IF NOT EXISTS `donate`.`tocken_details` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `verificationcode` INT UNSIGNED NULL,
  `date` DATE NULL,
  `time` DATE NULL,
  `donor_id` INT UNSIGNED NOT NULL,
  `hospital_id` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_donor_details_donor1_idx` (`donor_id` ASC),
  INDEX `fk_tocken_details_hospital1_idx` (`hospital_id` ASC),
  CONSTRAINT `fk_donor_details_donor1`
    FOREIGN KEY (`donor_id`)
    REFERENCES `donate`.`donor` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_tocken_details_hospital1`
    FOREIGN KEY (`hospital_id`)
    REFERENCES `donate`.`hospital` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
